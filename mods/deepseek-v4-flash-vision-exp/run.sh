...
# Preflight the staged overlay inside this container: py_compile catches syntax
# errors and importing vision_exp.processor catches API drift against the
# installed vLLM multimodal processing layer (e.g. the 2026-09-04 nightly
# rewrite of _apply_hf_processor_main). Fail here, not after vllm serve starts.
python3 -m py_compile "$PATCHES_DIR"/vision_exp/*.py
python3 -c "import sys; sys.path.insert(0, '$PATCHES_DIR'); import vision_exp.processor, vision_exp.apply, vision_exp.vision"
echo "$PREFIX vision_exp overlay imports cleanly against this container's vLLM"
